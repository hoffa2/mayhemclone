...
Every time a bullets position is updated a method inside the Lazer class called "outofbounds" checks if the bullet has wandered off the screen and if so it is removed from the spesific bulletlist. The same goes for when the bullet gets in the black hole.
